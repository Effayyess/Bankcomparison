...
import { authors, guideAuthorMap } from '../lib/authorsData';
...
// Extract FAQ items from guide sections — sections whose heading starts with common FAQ patterns
function extractFaqItems(guide: { sections: { heading: string; content: string }[] }) {
  return guide.sections
    ...
    .map((s) => ({
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: s.content.replace(/\*\*(.*?)\*\*/g, '$1').split('\n\n')[0].slice(0, 500),
      },
    }));
}
...
export default function GuidePage() {
  ...
  // Resolve author for this guide
  const authorId = guideAuthorMap[slug] || 'priya-sharma';
  const author = authors[authorId];

  // Build FAQ schema from guide sections
  const faqItems = extractFaqItems(guide);
  ...
  const guideSchema: object[] = [
    {
      ...
      speakable: {
        ...
        cssSelector: ['.guide-intro', 'h1', '.guide-section:first-of-type'],
      },
    },
  ];
          ...
          <div className="relative container py-10">
            ...
            {author && (
              <div className="flex items-center gap-3 mt-4 text-sm text-slate-400">
                ...
                />
                <span>
                  Written by <strong className="text-slate-200">{author.name}</strong>
                  <span className="mx-1.5 text-slate-600">·</span>
                  Last reviewed: <strong className="text-slate-200">{currentMonth}</strong>
                </span>
              </div>
            )}
          </div>
        ...
        <div className="container py-10">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <article className="flex-1 min-w-0">
              ...
              {/* Intro */}
              <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6 shadow-sm guide-intro">
              ...
              {guide.sections.map((section) => (
                <div key={section.id} id={section.id} className="bg-white border border-gray-200 rounded-xl p-6 mb-6 shadow-sm guide-section">
              ...
              <div className="mt-6 bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-wrap items-center justify-between gap-3 text-xs text-gray-500">
                <span>
                  Written by <strong className="text-gray-700">{author?.name}</strong>, {author?.role}
                  <span className="mx-1.5 text-gray-300">·</span>
                  Last reviewed: <strong className="text-gray-700">{currentMonth}</strong>
                </span>
            ...
            <aside className="hidden lg:block w-64 flex-shrink-0">
              <div className="sticky top-24 space-y-4">
                ...
                <div className="rounded-xl p-5" style={{ background: '#0f172a' }}>
                  ...
                  <p className="text-xs text-slate-400 mb-3">Compare all UK business bank accounts side by side.</p>
                  <Link
                    href="/compare"
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm text-center">
                  <p className="text-xs text-gray-500 mb-2">About this guide</p>
                  <Link href="/how-we-review" className="text-xs text-blue-600 hover:text-blue-800 font-medium no-underline">
